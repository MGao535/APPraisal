...
class Calendar extends React.Component {
 constructor(props){
  ...
  chooseColor(day){
    if(this.state.dayList.includes(day)){
        //console.log("true");
        return 'day';
      }
      if(this.state.appList.includes(day)){
        //console.log("true");
        return 'app';
      }
      if(this.state.preList.includes(day)){
        //console.log("true");
        return 'pre';
      }
      if(this.state.readyList.includes(day)){
        //console.log("true");
        return 'ready';
      }
  }
  ...
  renderCellsx(e) {
    ...
    while (day <= endDate) {
      for (let i = 0; i < 7; i++) {
        ...
        const cloneDay = day;
        days.push(
          <div className={`col cell ${!isSameMonth(day, monthStart) ? "disabled" : this.isInDays(day) ? "selected":""}`}
            key={day}
